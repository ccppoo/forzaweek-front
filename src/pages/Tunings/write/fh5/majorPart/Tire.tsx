...
export function FieldArrayTuningMajorPartsTire<T extends FieldValues>({
  register,
  formPath,
  watch,
}: {
  ...
  return (
    <Select<'normal' | 'drift' | 'rally' | 'offroad' | 'drag'>
    ...
    >
      {Suspension_choice.map((choice) => (
        <MenuItem key={`major-parts-tire-${choice.name}`} value={choice.name}>
