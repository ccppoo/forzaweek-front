...
import { BASE_PATH_REACTION_FH5_DECAL } from './config';
...
export async function GetVotesDecalImages({
  queryKey,
}: {
  ...
  const url = `${BASE_PATH_REACTION_FH5_DECAL}/image/${voteEndpointPath}`;
...
export async function DoVotesDecalImages({
  authToken,
  voteEndpointPath,
  vote,
}: {
  ...
  const url = `${BASE_PATH_REACTION_FH5_DECAL}/${voteEndpointPath}/${vote}`;
